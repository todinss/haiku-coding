...
class HaikuGenerator(MDApp):
    ...
    def generate_haiku(self):
        self.root.ids["verso_1"].text = choice(self.database['verso_1']).capitalize()
        self.root.ids["verso_2"].text = choice(self.database['verso_2']).lower()
        self.root.ids["verso_3"].text = choice(self.database['verso_3']).lower()
        self.add_to_history()
    ...
    def copy_haiku(self):
        ...
        if haiku != ["", "", ""] and  "\n".join(haiku) != Clipboard.paste():
            Clipboard.copy("\n".join(haiku))
            Snackbar(text=f"Elemento copiato negli appunti!").show()
    ...
    def change_theme(self): 
        ...
        if self.theme_cls.theme_style == "Dark":
            ...
            self.title_color = "000000"
            self.text_color = "181818"
        Snackbar(text=f"Il tema è stato modificato!").show()
    ...
    def delete_history(self, obj):
        ...
        self.history_dialog = None
        self.reload_history()
        Snackbar(text=f"La cronologia è stata eliminata!").show()
    ...
    def delete_item(self, obj):
        ...
        Snackbar(text=f"Elemento eliminato dalla cronologia!").show()
